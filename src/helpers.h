...
/**
 * Disassembles the binary instruction into human-readable assembly code, prints it out to
 * the console (NOTE: TODO: This will not work on Windows without attaching a console, so we
 * will need a suitable workaround that uses Visual Studio's OutputDebugString or whatever.)
 * and returns the size of the instruction.
 */

i32
DisassembleBinary(void* BinaryData)
{
	u8 iSize = 1;
